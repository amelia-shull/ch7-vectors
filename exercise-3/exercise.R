...
words <- c("the", "dog", "went", "for", "a", "walk")
...
# "g" through "m"
g.m.words <- words[words >= "g"]
g.m.words <- g.m.words[g.m.words <= "m"]
...
WordBin <- function(words, letter1, letter2) {
  new.words <- words[words >= letter1]  
  new.words <- new.words[new.words <= letter2]
  return(new.words)
}
